...
<div class="eventorganiser-event-meta group">
	...
	<ul class="eo-event-meta">
		...
		<?php if ( eo_get_venue() ) {
			...
			<li><a href="#map" class="venue-trigger"> <?php eo_venue_name(); ?></a></li>
		<?php } ?>
		...
		<?php if ( eo_recurs() ) {
				...
				if ( $upcoming->have_posts() ) : ?>
					...
					<li class="upcoming"><strong><?php _e( 'Upcoming Dates', 'eventorganiser' ); ?>:</strong>
						<ul class="eo-upcoming-dates">
							...
							while ( $upcoming->have_posts() ) {
								$upcoming->the_post();
								if($i == 1) {
									echo '<li class="first-event">';
								} else {
									echo '<li>';
								}
								echo eo_format_event_occurrence() . '</li>';
								$i++;
							};
							?>
						</ul>
